...
Private Sub logcat_LogCatData (Buffer() As Byte, Length As Int)
	logs.Append(BytesToString(Buffer, 0, Length, "utf8") & CRLF)
...
'	Else
'		SendEmail($"خطا در اپ فروشگاه ${Library.GetStringResourse("app_title")}"$,ms & temp)
